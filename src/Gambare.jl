...
mutable struct Game{R, RNG} <: GW.AbstractGridWorld
    tile_map::BitArray{3}
...
function Game(; R = Float32, height = 8, width = 8, rng = Random.GLOBAL_RNG)
    ...
    env = Game(tile_map, agent_position, reward, rng, done, terminal_reward, goal_position)
...
function GW.act!(env::Game, action)
...
GW.get_height(env::Game) = size(env.tile_map, 2)
GW.get_width(env::Game) = size(env.tile_map, 3)

GW.get_action_names(env::Game) = (:MOVE_UP, :MOVE_DOWN, :MOVE_LEFT, :MOVE_RIGHT)
GW.get_object_names(env::Game) = (:AGENT, :WALL, :GOAL)

function GW.get_pretty_tile_map(env::Game, position::CartesianIndex{2})
    characters = ('☻', '█', '♥', '⋅')
...
function GW.get_pretty_sub_tile_map(env::Game, window_size, position::CartesianIndex{2})
    tile_map = env.tile_map
    agent_position = env.agent_position
...
function Base.show(io::IO, ::MIME"text/plain", env::Game)
    str = "tile_map:\n"
...
GW.get_action_keys(env::Game) = ('w', 's', 'a', 'd')
